...
Even before calculating the actual p-value we know that with a t stat over 4 standard deviations away from the hypothesized value that it is very unlikely to happen. The p-value we get when testing our t-stat is smaller than .001, which is strong evidence that our null hypothesis is wrong, and so we do not believe that demand is own-price unit elastic.
